...
function createLazyComponent<T extends ComponentType<any>>(
  ...
  return function LazyMountedComponent(props: React.ComponentProps<T>) {
...
/* ────────────────────────────────────────────────────────────────────────────
   ...
   FIXED:
   - DropdownMenuLabel must be inside DropdownMenuRadioGroup or DropdownMenuGroup.
──────────────────────────────────────────────────────────────────────────── */
...
function MapLocateControl({
  ...
  function startLocating() {
    ...
    map.on("locationfound", (locationEvent: LocationEvent) => {
      setLocation(locationEvent.latlng);
      setIsLocating(false);
...
function MapDrawActionButton<T extends EditToolbar.Edit | EditToolbar.Delete>({
  ...
  useEffect(() => {
    if (!L || !featureGroup || !isActive) {
      (controlRef.current as any)?.disable?.();
      controlRef.current = null;
      return;
    }

    const control = createDrawTool(L, map as DrawMap, featureGroup);
    (control as any)?.enable?.();
    controlRef.current = control;

    return () => {
      (control as any)?.disable?.();
      controlRef.current = null;
    };
  }, [L, map, isActive, featureGroup, createDrawTool]);

  function handleClick() {
    (controlRef.current as any)?.save?.();
...
function MapDrawEdit({
  ...
  useEffect(() => {
    ...
    if (LAny.Edit?.PolyVerticesEdit?.mergeOptions) {
      LAny.Edit.PolyVerticesEdit.mergeOptions({
        icon: mapDrawHandleIcon,
        touchIcon: mapDrawHandleIcon,
        drawError: {
          color: "var(--color-destructive)",
    ...
    if (LAny.Edit?.SimpleShape?.mergeOptions) {
      LAny.Edit.SimpleShape.mergeOptions({
    ...
    if (LAny.drawLocal?.edit?.handlers?.edit) {
      LAny.drawLocal.edit.handlers.edit.tooltip = {
        text: "Drag handles or markers to edit.",
        subtext: "",
      };
    }

    if (LAny.drawLocal?.edit?.handlers?.remove) {
      LAny.drawLocal.edit.handlers.remove.tooltip = {
...
function useLeaflet() {
  ...
  useEffect(() => {
    async function loadLeaflet() {
      ...
      const L_object = (leaflet.default ?? leaflet) as typeof import("leaflet");
      ...
      setLeafletDraw(leafletDraw as unknown as typeof import("leaflet-draw"));
      setL(L_object);
    }
